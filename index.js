...
function get20s(array){
  ...
  for(let i = 0; i < array.length; i++) {
      let yearRange = array[i].years.split(" - ");
      if(parseInt(yearRange[0]) > 1899 && parseInt(yearRange[1]) < 2000) {
        dateChecker.push(array[i].name);
      }
  }
  return dateChecker
}
...
function lotsOfArt(array){
  const lotsOPaintings = [];
  for(let i = 0; i < array.length; i++) {
    if(array[i].paintings > 100) {
      lotsOPaintings.push(array[i].name);
    }
  }
  return lotsOPaintings;
}
...
function getHTML(data){
    ...
    for(x in data) {
        ...
        <div class ="${data[x].name}">
        <a href="${data[x].wikipedia}"> ${data[x].name}</a>
        </div>
        <div class = "bio">${data[x].bio}</div>
        </div>`
